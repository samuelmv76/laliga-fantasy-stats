...
import { TEAMS } from './mockPlayers.tsx'
import type { FixturesByTeam, Odds } from '../types.tsx'

const NEXT_MATCHDAYS = [6, 7, 8, 9]
const REFERENCE_KICKOFF = new Date('2026-09-20T21:00:00+02:00')

function kickoffFor(matchdayIndex: number, teamIndex: number): string {
...
function mockOdds(difficulty: number, home: boolean): Odds {
  const win = [0.62, 0.52, 0.42, 0.33, 0.25][difficulty - 1] + (home ? 0.05 : -0.05)
  const draw = 0.25
  const margin = 1.06
  const round = (p: number) => Math.round((1 / (p * margin)) * 100) / 100
  return { home: round(home ? win : 1 - win - draw), draw: round(draw), away: round(home ? 1 - win - draw : win) }
}

export const FIXTURES: FixturesByTeam = Object.fromEntries(
