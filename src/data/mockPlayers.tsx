...
import type { Player, PlayerStatus, Position } from '../types.tsx'

const BASE_PLAYERS: { id: number; name: string; team: string; pos: Position; basePrice: number }[] = [
...
export const POSITIONS: Position[] = ['POR', 'DEF', 'MID', 'DEL']
...
export const PLAYER_STATUS: Record<PlayerStatus, { label: string; tone: 'warn' | 'danger' }> = {
...
function seededWalk(seed: number, days: number, basePrice: number): number[] {
...
export const PLAYERS: Player[] = BASE_PLAYERS.map((p) => {
  ...
  const status: PlayerStatus | undefined =
    p.id % 7 === 0 ? 'lesion' : p.id % 11 === 0 ? 'sancion' : p.id % 5 === 0 ? 'duda' : undefined

  return { ...p, id: String(p.id), priceHistory, pointsByMatchday, points, status }
})

export function currentPrice(player: Player): number {
  return player.priceHistory[player.priceHistory.length - 1].price
}

export function todayDelta(player: Player): number {
...
export function last(player: Player, n: number) {
