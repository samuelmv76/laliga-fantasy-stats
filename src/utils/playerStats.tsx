...
import { fixtureDifficulty } from './opponent.tsx'
...
export function matchesPlayed(player: Player): number {
...
export function pointsAverage(player: Player): number {
...
export function form(player: Player, matchdays = 3): number {
  const recent = (player.pointsByMatchday ?? []).slice(-matchdays)
  return recent.reduce((sum, m) => sum + m.points, 0)
}

export function pointsPerMillion(player: Player): number {
...
export function priceDelta(player: Player, days = 1): number {
...
export function fromPeak(player: Player): number {
  const history = player.priceHistory ?? []
...
export function nextDifficulty(teamFixtures: Fixture[] | undefined, count = 3): number | null {
...
export type DifficultyBand = (typeof DIFFICULTY_BANDS)[number]['id']

export function difficultyBand(difficulty: number | null | undefined): DifficultyBand | null {
  if (difficulty == null) return null
  return DIFFICULTY_BANDS.find((band) => difficulty <= band.max)?.id ?? null
}
