...
import {
  ...
  numberOrNull,
  sortPlayers,
} from './playerFilters.tsx'
import { jugador as construirJugador, partido } from './testData.tsx'
import type { Player } from '../types.tsx'
import { test } from 'vitest'

test('playerFilters.tsx: filtros, búsqueda, calendario y orden', () => {
  // Igual que el constructor compartido, pero con el precio en millones, que
  // es lo que miran estos filtros.
  function jugador({ price = 10, ...extra }: Partial<Player> & { id: string; price?: number }): Player {
    return construirJugador({
      ...extra,
      priceHistory: [
        { date: '2026-09-01', price: 8_000_000 },
        { date: '2026-09-02', price: price * 1_000_000 },
      ],
    })
  }
  ...
  const ids = (list: Player[]) => list.map((p) => p.id).join(',')
