...
import type { DifficultyBand } from './playerStats.tsx'
import type { FixturesByTeam, Player, Position } from '../types.tsx'
import {
  difficultyBand,
  form,
  fromPeak,
  ...
  pointsPerMillion,
  priceDelta,
} from './playerStats.tsx'

type Sorter = (a: Player, b: Player) => number
...
export type SortKey = keyof typeof SORTERS | typeof SORT_BY_DIFFICULTY
...
export interface Filters {
  positions: Position[]
  teams: string[]
  /** 'ok' es el jugador sin ninguna baja; el resto son valores de PlayerStatus. */
  statuses: string[]
...
export type FilterCounts = Record<string, number> & { total: number }

export function filterCounts(players: Player[]): FilterCounts {
  const counts: FilterCounts = { total: players.length }
...
// una vez por jugador.
export type DifficultyByTeam = Record<string, number | null>

export function difficultyByTeam(teams: string[], fixtures?: FixturesByTeam): DifficultyByTeam {
  const byTeam: DifficultyByTeam = {}
...
export function filterPlayers(
  players: Player[],
  filters: Filters,
  { query = '', difficulty = {} }: { query?: string; difficulty?: DifficultyByTeam } = {}
): Player[] {
...
export function sortPlayers(players: Player[], sortKey: SortKey, difficulty: DifficultyByTeam = {}): Player[] {
  if (sortKey === SORT_BY_DIFFICULTY) {
    // Sin calendario, al final: no es "fácil", es que no se sabe.
    const value = (player: Player) => difficulty[player.team] ?? Infinity
