...
import { jugador, partido } from './testData.tsx'
import { test } from 'vitest'

test('playerStats.tsx: media, forma, precio, calendario y tramos', () => {
  ...
  assert.equal(matchesPlayed(jugador({ id: '2', points: 0, played: undefined, pointsByMatchday: [{ matchday: 1, points: 3 }] })), 1)

  // Sin partidos jugados la media es 0, nunca NaN ni Infinity.
  assert.equal(pointsAverage(jugador({ id: '3', points: 0, played: 0, pointsByMatchday: [] })), 0)

  // Forma: solo las últimas 3 jornadas (-2 + 8 + 8), no el total.
  assert.equal(form(player), 14)
  assert.equal(form(player, 1), 8)
  assert.equal(form(jugador({ id: '4', points: 5, pointsByMatchday: undefined })), 0)
  ...
  assert.equal(priceDelta(jugador({ id: '5', priceHistory: [{ date: '2026-09-01', price: 5 }] })), 0)

  // Está un 8,33% por debajo de su máximo (11M sobre 12M).
  assert.ok(Math.abs(fromPeak(player) - (11 / 12 - 1)) < 1e-9)
  assert.equal(fromPeak(jugador({ id: '6', priceHistory: [] })), 0)

  assert.equal(pointsPerMillion(player), 24 / 11)

  // Calendario: media de dificultad de los próximos partidos (por cuotas).
  const facil = partido({ home: true, opponent: 'Getafe', odds: { home: 1.4, draw: 4.8, away: 7.5 } })
  const dificil = partido({ home: false, opponent: 'Real Madrid', odds: { home: 1.4, draw: 4.8, away: 7.5 } })
